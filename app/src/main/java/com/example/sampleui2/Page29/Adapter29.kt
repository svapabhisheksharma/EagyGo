...
class Adapter29(var list:List<Model29>): RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        when(viewType) {
            0 -> {
                val view = LayoutInflater.from(parent.context).inflate(R.layout.normal_tile, parent, false)
                return Viewholder0(view)
            }
            1 -> {
                val view = LayoutInflater.from(parent.context).inflate(R.layout.spinner_tile, parent, false)
